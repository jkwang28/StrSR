...
def print_vram_state(msg, logger=None):
    """Log current device VRAM usage and return metrics.

    Note: In distributed training each process logs its own device only.
    """
    ...
    if logger:
        logger.info(
            ...
            f"reserved = {cache:.2f} GB, peak = {peak:.2f} GB"
        )
...
def module_param_memory(module: torch.nn.Module, only_trainable: bool = False, include_buffers: bool = True) -> int:
    """Return parameter (and optionally buffer) memory in bytes for a module.

    This computes actual memory footprint on device given each tensor's dtype.
    """
    total = 0
    for p in module.parameters(recurse=True):
        if only_trainable and not p.requires_grad:
            continue
        total += p.numel() * _dtype_num_bytes(p.dtype)
    if include_buffers:
        for b in module.buffers(recurse=True):
            total += b.numel() * _dtype_num_bytes(b.dtype)
    return total


def estimate_adamw_state_bytes(param_bytes: int, state_dtype: torch.dtype = torch.float32) -> int:
    """Estimate AdamW optimizer states size in bytes.

    AdamW keeps two states (exp_avg and exp_avg_sq). Many implementations use fp32 states.
    """
    return 2 * param_bytes if _dtype_num_bytes(state_dtype) == 4 else int(2 * param_bytes * _dtype_num_bytes(state_dtype) / 4)
